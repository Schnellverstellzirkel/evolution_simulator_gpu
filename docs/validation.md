...
The self-collision check uses a five-node chain with two nonadjacent bones crossing exactly. CPU and GPU both attempt to unfold the skeleton, preserve all four exact rest lengths, and mark any remaining capsule penetration as a failed trial; the GPU score test confirms failed specimens receive `FAILED` and cannot enter the archive as elites. A separate CPU case verifies that adjacent bones can separate after overlapping outside their joint caps. The QD version is bumped so loading an older checkpoint clears its previously scored archive and reevaluates the saved population under the new collision rule. This adds pairwise bone checks to the physics step; no throughput profile was run while the CPU/GPU were busy.
