...
A batch keeps its transient state in reusable GPU storage buffers between dispatches. Responsive mode limits dispatches to 64 steps; throughput mode uses 1024. Encoding and parameter uploads overlap queued work; up to two responsive-mode or eight throughput-mode compute submissions can be in flight. Fitness and behavior measurements are read back as one compact record per creature; full node readback is used only for replay validation. Population creation and offspring generation use CPU parallelism. Archive insertion and emitter feedback run once after a complete population batch.
