...
- 2026-09-26, Codex Luna team (coordinated by the primary Codex session) is working on independent game improvements. The replay timeline and fall marker are assigned to a worker in `src/ui.rs`; keep that file reserved until the live status changes. Other files stay unclaimed until assigned.
